...
5. **Position flex** — versatile players can fill more than one spot (a pass-catching RB or a fast, route-running TE can play WR; a big red-zone WR can play TE), and you choose where they line up.
...
After your lineup is set, a dedicated final-pick page appears: choose one captain from four candidates drawn from teams you haven't used — always at least one DB, one LB, and one D-lineman, plus a wildcard.

**The captain's position changes how your defense plays — and your score.** Every simulated opponent leans pass-heavy or run-heavy:
...
The three roles have nearly equal expected value (verified over 20,000 simulated seasons), so the pick is a playstyle bet, not a "correct answer." The final-roster screen explains what your captain does for the team.
...
- **Daily Leaderboard** — post your score under any name and see today's top 20 runs (one entry per device per day). Powered by Supabase with row-level security.
- **Free Play** — unlimited games, fresh randomness every time.
- **Classic vs Expert** — Classic shows 0-99 overall and attribute ratings (Madden-style: ARM/ACC/AWR for QBs, COV/INT/TAK for DBs, etc.). Expert hides every rating — you scout real stat lines (yards, TDs, sacks, INTs) and draft on football knowledge alone.
...
- **Legends (pre-1999):** 108 hand-rated all-time greats on the same scale — Joe Montana, Dan Marino, Barry Sanders, Walter Payton, Jim Brown, Jerry Rice, Reggie White, Lawrence Taylor, Dick Butkus, Ronnie Lott, Deion Sanders, Jack Lambert, Night Train Lane, the '85 Bears defense, The Hogs O-line, and more. Legends carry a gold badge in the draft.

Ratings are **era-relative**: a player is measured against his own season's league (z-scores per position per year, re-standardized across years), so a 1999 star and a 2024 star are directly comparable. An average starter sits around 76; the elite cap is 99.

## The simulation

- 17 regular-season games against a league of varying strength, then (with 10+ wins) a three-round postseason against progressively tougher opponents — Divisional, Conference, Super Bowl.
- Each game: your roster average + captain matchup bonus + noise vs. the opponent's roll. Better rosters win more; nobody is safe every week.
- Results land on a ladder from **Relegation** up through **Super Bowl Champion** and finally **20-0 — Perfect. Immortal.** 🐐

## Design

Vintage football-program aesthetic: aged-paper palette, varsity block display font (Graduate), hard-offset ink shadows, chalkboard team banners, and a newspaper-masthead header. No gradients, no glassmorphism — it should feel like a 1970s game-day program, not an app.
...
| Daily seeding | mulberry32 PRNG seeded from the date — fully deterministic, no server |
...
Everything except the leaderboard is fully static — there is no game server. Daily challenges are reproducible on any device from the date alone.
...
Pushing to `main` triggers the GitHub Actions workflow (`.github/workflows/deploy.yml`), which builds and publishes to GitHub Pages automatically. The site URL lives in one place — `VITE_SITE_URL` in `.env` — and flows into the share links, OG tags, canonical URL, and structured data at build time.
